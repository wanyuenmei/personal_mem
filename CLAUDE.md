...
- **No milestone references.** Don't cite internal roadmap milestones (`M2.2`, `M4`) anywhere — code, docstrings, PR titles, PR bodies. They age badly and mean nothing to a future reader. Name the thing ("the OAuth layer", "the consent layer") and use Linear ids (`VC-N`) as pointers. The team key was `PER` until 2026-07-30; existing `PER-N` references are the same issues (Linear redirects them) and stay as-is — don't retroactively rewrite them.
...
- **Title:** `<ticket>: <short description>`, e.g. `VC-86: add the consent-scope registry and register_scopes tool`. A correction too small to warrant its own Linear issue may use `fix: <short description>` and omit the **Ticket** section. Reach for a ticket by default — `fix:` is the exception, not a way to avoid writing one.
...
- **Branch naming:** `<user>/<ticket>-<slug>`, e.g. `wanyuenmei/vc-86-consent-scope-registry`.
