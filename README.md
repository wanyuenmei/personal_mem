...
**Map** is a third tab alongside **In your context** and **Set aside**, showing the same memories as a mind map: you at the centre, your scopes around you sized by how much is filed under each, and whatever nothing has claimed yet as its own node. Click a scope to fan its memories out as leaves and dim the rest; the cards themselves appear under the map. **Summarize my scopes** fills in a sentence or two per scope describing what the store actually holds there — the thing you would be handing over by sharing that scope, which a name and a count can't tell you. It is one model call for the whole vocabulary, runs only when you press it, and needs `EXTRACTION_MODE=anthropic`; the map itself works in every mode, just without the summaries.
...
Both passes are **durable**: pressing the button records a run, and a background worker executes it. Restarting or redeploying the server mid-pass no longer loses it — the next worker to start picks the run up and carries on, and the page keeps showing it because the progress is stored rather than held in whichever process served the click. Each memory also records which vocabulary it was last classified against, so a resumed pass only pays for what the interrupted one hadn't reached, and pressing the button again over an unchanged store costs almost nothing. Changing a scope's name or description changes what "current" means, and the next pass re-classifies everything, which is the point.
