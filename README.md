...
**Claude web · Cursor · ChatGPT (remote)** — deploy it (below) with WorkOS configured, then paste the plain URL into each app's connector/MCP settings:

```
https://<your-domain>/mcp
```

Whoever adds it is bounced to a WorkOS sign-in and lands in their own isolated namespace (`WORKOS_USER_ID_PREFIX` + the token's subject), so one deployment can serve several people without any of them seeing another's memories. Setup is in [OAuth mode](#oauth-mode-workos-authkit) below.
...
Do this once per client, right after connecting it above.
...
`PORT` is injected automatically. Then `railway up` (builds the Dockerfile), and **Settings → Networking → Generate Domain** for a public URL — your MCP endpoint is `https://<domain>/mcp`, or `https://<domain>/<token>/mcp` in capability mode. Liveness check: `curl -s https://<domain>/health` → `ok`. Paste the URL into each client's connector settings (see above).
